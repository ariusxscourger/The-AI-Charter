      ...
      {[1, 2, 3].map((bar) => (
        <div
          ...
          className={`w-1.5 rounded-sm transition-all duration-300 ${
            bar <= bars ? "bg-[#1F2937]" : "bg-[#C7C7C7]/40"
          }`}
    ...
    switch (verdict) {
      case "approved":
        return {
          ...
          icon: <CheckCircle className="h-10 w-10 stroke-[1.5px] text-[#76E1A7] drop-shadow-[0_0_8px_rgba(118,225,167,0.6)]" />,
        }
      case "rejected":
        return {
      ...
      case "conditional_approval":
        return {
          ...
          icon: <AlertTriangle className="h-10 w-10 stroke-[1.5px] text-orange-400 drop-shadow-[0_0_8px_rgba(249,115,22,0.6)]" />,
        }
      default:
        return {
    ...
    <div className="relative min-h-screen bg-[#FAF8F5] pb-24 font-sans text-[#1F2937]">
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-6 pt-12">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="flex flex-col justify-between gap-4 overflow-hidden rounded-xl border border-[#C7C7C7]/20 bg-[#1F2937] p-8 shadow-2xl sm:flex-row sm:items-center relative">
            ...
            <div className="relative z-10 text-left font-mono text-xs sm:text-right">
              ...
              <span className="mt-1 block font-bold text-[#A1DFF5] text-lg tracking-wider drop-shadow-sm">{record.referenceId}</span>
            </div>
          </motion.div>

          {/* 1. Verdict Showcase Block */}
          <motion.div variants={itemVariants}>
            <Card className={`relative flex flex-col items-center gap-6 overflow-hidden rounded-xl border-2 p-8 transition-shadow duration-500 md:flex-row ${verdictConfig.bg}`}>
              {/* Glassy sheen */}
              ...
              <div className="relative z-10 flex-1 space-y-3 text-center md:text-left">
                <div className="flex flex-col items-center justify-center gap-3 md:flex-row md:justify-start">
                  ...
                  <Badge variant={verdictConfig.badge} className="px-3 py-1 text-sm shadow-sm">{record.verdict.replace(/_/g, " ")}</Badge>
                </div>
            ...
            <div className="grid grid-cols-1 overflow-hidden rounded-xl border border-[#C7C7C7]/80 bg-[#FAF8F5] shadow-sm font-sans text-xs md:grid-cols-3">
              ...
              <div className="space-y-6 border-[#C7C7C7]/50 border-r bg-[#1F2937]/5 p-6">
                <div className="space-y-1.5">
                  <span className="block font-bold font-mono text-[#1F2937]/50 text-[10px] uppercase tracking-widest">Feature Type</span>
                  <span className="block font-bold font-mono text-sm uppercase text-[#1F2937]">
                    {record.submission.featureType.replace(/_/g, " ")}
                  </span>
                </div>
                <div className="space-y-1.5">
                  <span className="block font-bold font-mono text-[#1F2937]/50 text-[10px] uppercase tracking-widest">PII Involved</span>
                  <span className="block font-bold font-mono text-sm uppercase text-[#1F2937]">{record.submission.piiInvolved}</span>
                </div>
                <div className="space-y-2">
                  <span className="block font-bold font-mono text-[#1F2937]/50 text-[10px] uppercase tracking-widest">Affected Systems</span>
                ...
                <div className="space-y-2">
                  <span className="block font-bold font-mono text-[#1F2937]/50 text-[10px] uppercase tracking-widest">Target Jurisdictions</span>
                  <div className="flex flex-wrap gap-2">
              ...
              <div className="space-y-6 p-6 md:col-span-2 bg-white/50 backdrop-blur-sm">
                <div className="space-y-2">
                  <span className="block font-bold font-mono text-[#1F2937]/50 text-[10px] uppercase tracking-widest">Description</span>
                  <p className="text-[#1F2937]/80 leading-relaxed font-medium text-sm">{record.submission.description}</p>
                </div>
                <div className="space-y-2">
                  <span className="block font-bold font-mono text-[#1F2937]/50 text-[10px] uppercase tracking-widest">Intended Use</span>
                  <p className="text-[#1F2937]/80 leading-relaxed font-medium text-sm">{record.submission.intendedUse}</p>
                </div>
                <div className="space-y-2">
                  <span className="block font-bold font-mono text-[#1F2937]/50 text-[10px] uppercase tracking-widest">Data Sources</span>
                  <p className="text-[#1F2937]/80 leading-relaxed font-medium text-sm">{record.submission.dataSources}</p>
                </div>
              </div>
            </div>
          </motion.div>
              ...
              {record.agentRecords.map((agent) => {
                ...
                const titleElement = (
                  <div className="flex w-full flex-wrap items-center justify-between gap-4 pr-4 font-sans text-sm">
                    <div className="flex items-center gap-3">
                      <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#1F2937]/5 text-lg" role="img" aria-label={agent.name}>
                        {agent.emoji}
                      </span>
                      <span className="font-bold text-[#1F2937] tracking-tight">{agent.name}</span>
                    </div>
                    <div className="flex items-center gap-6 font-mono text-xs">
                      <Badge variant={getVerdictVariant(agent.vote)} className="shadow-sm">{agent.vote}</Badge>
                  ...
                  >
                    ...
                    >
                      <div className="space-y-5 px-1 py-2">
                        {/* Reasoning text */}
                        <div>
                          <div className="mb-2 font-bold font-mono text-[#1F2937]/50 text-[10px] uppercase tracking-widest">Reasoning Statement</div>
                              ...
                              {agent.findings.map((finding) => (
                                ...
                                >
                                  <div className="flex items-center justify-between gap-2">
                                    <span className="font-bold font-mono text-[#1F2937] text-xs uppercase tracking-wider">{finding.title}</span>
                                    <Badge variant={getSeverityVariant(finding.severity)}>{finding.severity}</Badge>
                                  </div>
                                  <p className="font-sans font-medium text-[#1F2937]/80 text-sm leading-relaxed">{finding.detail}</p>
            ...
            <motion.div variants={itemVariants} className="space-y-4">
              ...
              <Card className="space-y-8 rounded-xl border border-[#C7C7C7]/80 bg-white p-8 shadow-sm">
                {record.crossExaminationLog.map((log, idx) => (
                  ...
                  >
                    {/* Timeline bullet */}
                    <span className="absolute top-1.5 -left-[9px] flex h-4 w-4 select-none items-center justify-center rounded-full border-[3px] border-white bg-[#1F2937] shadow-sm">
                    </span>
                    ...
                    <div className="grid grid-cols-1 gap-5 font-sans text-sm md:grid-cols-2">
                      <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-4 shadow-sm">
                        <div className="mb-2 font-bold font-mono text-red-500/70 text-[10px] uppercase tracking-widest">Challenge Posted</div>
          ...
          <motion.div variants={itemVariants} className="flex flex-col items-center justify-between gap-4 border-[#C7C7C7]/40 border-t-2 pt-8 font-mono text-[#1F2937]/50 text-[10px] sm:flex-row tracking-widest">
